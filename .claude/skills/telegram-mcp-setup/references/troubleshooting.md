...
Имя приложения должно быть уникальным глобально. Добавить цифры/инициалы: `claudecode2026`.

### ERROR при создании приложения (реальный кейс)

При нажатии "Create application" форма возвращает голое «ERROR» без деталей. По опыту команды причина почти всегда одна из четырёх, по порядку проверки:

1. **Short name не строго alphanumeric.** Разрешены только латинские буквы и цифры — БЕЗ подчёркиваний (`_`), дефисов (`-`), пробелов, кириллицы. `claude_2026` и `claude-code` вызовут ERROR, `claudecode2026` — нет.
2. **App title не латиницей или с пробелами.** Использовать латиницу без пробелов, например `ClaudeCodeIntegration` (можно CamelCase, без разделителей).
3. **Включён VPN на устройстве.** my.telegram.org нестабильно обрабатывает форму через VPN — выключить VPN полностью (не переключить сервер) и повторить.
4. **Fallback — крайний вариант, умеренный риск.** Если после пп. 1-3 всё равно ERROR (или нет доступа создать своё приложение) — временно использовать api_id/api_hash коллеги, у которого уже есть рабочее приложение на my.telegram.org. Session string при этом всё равно генерируется под СВОИМ номером телефона в Шаге 2 — коллега делится только `api_id`/`api_hash`, не доступом к своему аккаунту. Риск: оба использует один api_id — при отзыве/бане credentials коллеги отвалится и твоё подключение. Заменить на собственные credentials, как только появится возможность создать своё приложение.
